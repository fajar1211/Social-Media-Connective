...
function AIContentTab({ client }: { client: { id: string; name: string; socialIntegrations?: Record<string, { connected?: boolean }> } }) {
  ...
  const toggleKnowledge = (type: "notes" | "files", value: string) => {
    const setter = type === "notes" ? setKnowledgeNotes : setKnowledgeFiles;
    setter((prev) =>
      ...
      <div className="rounded-xl border bg-card p-6 shadow-soft">
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="rounded-lg border p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Select knowledge sources to include in content generation.
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              {knowledgeOptions.map((option) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={knowledgeNotes.includes(option)}
                    onChange={() => toggleKnowledge("notes", option)}
                    className="size-4 rounded border-gray-300"
                  />
              ...
              {knowledgeOptions.map((option) => (
                ...
                >
                  <input
                    ...
                    onChange={() => toggleKnowledge("files", option)}
                    className="size-4 rounded border-gray-300"
                  />
                  <span className="text-sm">{option}</span>
                </label>
