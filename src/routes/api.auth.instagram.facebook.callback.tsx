...
export const Route = createFileRoute("/api/auth/instagram/facebook/callback")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (error) {
          return new Response(
            buildHtml({
              title: "Instagram via Facebook OAuth Error",
              body: `
                <h2>Instagram via Facebook OAuth Error</h2>
        ...
        if (!code) {
          return new Response(
            buildHtml({
              title: "Instagram via Facebook OAuth",
              body: `
                <h2>Instagram via Facebook OAuth Callback</h2>
        ...
        try {
          ...
          try {
            ...
            console.log("[InstagramFacebookCallback] Long-lived exchange response:", JSON.stringify(llExchangeData));
